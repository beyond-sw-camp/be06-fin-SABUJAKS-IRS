        ...
        <div id="content">
            ...
            <div class="detail-section">
                ...
                <p v-if="announcementDetail.announcementStart && announcementDetail.announcementEnd">
                    <strong>공고 기간:</strong> {{ announcementDetail.announcementStart }} - {{ announcementDetail.announcementEnd }}
                </p>
                <p v-if="announcementDetail.jobCategory"><strong>직무 카테고리:</strong> {{ announcementDetail.jobCategory }}</p>
                <p v-if="announcementDetail.jobTitle"><strong>모집분야명:</strong> {{ announcementDetail.jobTitle }}</p>
                <p v-if="announcementDetail.recruitedNum"><strong>모집 인원:</strong> {{ announcementDetail.recruitedNum }}명</p>
                <p v-if="announcementDetail.careerBase"><strong>경력 구분:</strong> {{ announcementDetail.careerBase }}</p>
                <p v-if="announcementDetail.positionQuali"><strong>포지션 & 자격 요건:</strong> {{ announcementDetail.positionQuali }}</p>
                ...
                <p v-if="announcementDetail.conditions"><strong>근무 조건:</strong> {{ announcementDetail.conditions }}</p>
                <p v-if="announcementDetail.benefits"><strong>복리후생:</strong> {{ announcementDetail.benefits }}</p>
                <p v-if="announcementDetail.imgUrl"><strong>공고 이미지:</strong> <img :src="announcementDetail.imgUrl" alt="공고 이미지" /></p>
                <br>
                <h2>담당자 정보</h2>
                <p v-if="announcementDetail.managerName"><strong>담당자명:</strong> {{ announcementDetail.managerName }}</p>
                <p v-if="announcementDetail.managerContact"><strong>담당자 연락처:</strong> {{ announcementDetail.managerContact }}</p>
                <p v-if="announcementDetail.managerEmail"><strong>담당자 이메일:</strong> {{ announcementDetail.managerEmail }}</p>
                
                <br>
                <h2>기업 정보</h2>
                <p>(더 자세한 기업 정보는 마이페이지에서 확인하세요)</p>
                <p v-if="announcementDetail.companyName"><strong>기업명:</strong> {{ announcementDetail.companyName }}</p>
                <p v-if="announcementDetail.companyIndustry"><strong>산업:</strong> {{ announcementDetail.companyIndustry }}</p>
                <p v-if="announcementDetail.companyBusiness"><strong>주요 사업:</strong> {{ announcementDetail.companyBusiness }}</p>
                <p v-if="announcementDetail.companyAddress"><strong>기업 주소:</strong> {{ announcementDetail.companyAddress }}</p>
                <p v-if="announcementDetail.intro"><strong>기업 소개:</strong> {{ announcementDetail.intro }}</p>
                
                <br>
                <h2>채용 절차</h2>
                <p v-if="announcementDetail.interviewNum"><strong>면접 횟수:</strong> {{ announcementDetail.interviewNum }}</p>
