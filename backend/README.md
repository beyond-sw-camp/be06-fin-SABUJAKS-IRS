...
> 화상 면접 대기방은 /video-interview/{공고UUID}로<br>
> 화상 면접 방은 /video-interview/{공고UUID}/{면접일정UUID}로 아무나 접근 가능했습니다.<br>
...
#### 도입 후
➡ 화상 면접 대기방에 접근
- 스프링 시큐리티의 접근제어를 통해 InterviewPaticipate 테이블을 조회해 권한을 확인하는 함수<br>
- 공고 UUID 및 면접 일정 UUID가 지원자에게 있다면 접근을 허용<br>
- 공고 UUID 및 면접 일정 UUID가 지원자에게 없다면 접근을 거부<br>
![지원자-화상면접방 공고별 접근제어](https://github.com/user-attachments/assets/c525fa29-a5c1-4dde-9ef4-49c28a1983ad)
➡ 화상 면접 방에 접근
- 화상 면접 방에 참가할 수 있는 Session-Token 발행시 시간대 확인 함수를 통해 면접참여 및 면접스케줄 테이블을 조회해 면접 접속 시간을 확인<br>
- 서버시간과 확인한 값을 비교해 3분전에 접속 가능하게 만든 함수<br>
![지원자-화상면접방 시간대 접근제어6](https://github.com/user-attachments/assets/498cdc58-01e9-47a3-a1b1-4d56a8e261e0)
👍 개선점<br>
1. 코드 단에서 접근제어 함수의 DB 조회가 많이 발생<br>
2. 시간대 접근제어 부분은 서비스 단에서 시행됨, 스프링 시큐리티가 이를 위임해야함<br>
