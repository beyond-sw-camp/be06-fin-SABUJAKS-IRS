...
import com.sabujaks.irs.domain.announcement.model.response.CompanyInfoRes;
import com.sabujaks.irs.domain.announcement.model.response.CustomFormCreateRes;
import com.sabujaks.irs.domain.announcement.model.response.ReadInfoRes;
...
//        Long recruiterIdx = customUserDetails.getIdx();
    ...
    public ResponseEntity<BaseResponse<ReadInfoRes>> readRecruiterInfo(
            Long recruiterIdx) throws BaseException {

//        @AuthenticationPrincipal CustomUserDetails customUserDetails 매개변수 추가
//        if (customUserDetails == null) throw new BaseException(BaseResponseMessage.AUTH_FAIL);
//        Long recruiterIdx = customUserDetails.getIdx();
        ReadInfoRes response = announcementService.readRecruiterInfo(recruiterIdx);
    ...
    public ResponseEntity<BaseResponse<CompanyInfoRes>> readCompanyInfo(
            Long recruiterIdx) throws BaseException {

//        @AuthenticationPrincipal CustomUserDetails customUserDetails 매개변수 추가
//        if (customUserDetails == null) throw new BaseException(BaseResponseMessage.AUTH_FAIL);
//        Long recruiterIdx = customUserDetails.getIdx();
        CompanyInfoRes response = announcementService.readCompanyInfo(recruiterIdx);
