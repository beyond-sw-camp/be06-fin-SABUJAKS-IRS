...
public enum BaseResponseMessage {
    ...
    NOT_FOUND(false, 404, "요청 URL을 찾을 수 없습니다."),
    EMAIL_SEND_FAIL(false, 311, "이메일 전송에 실패했습니다."),
    INTERNAL_SERVER_ERROR(false, 500, "내부 서버 오류로 처리할 수 없습니다."),
    // 10단위로 성공

    // [AUTH] 1000~1999
    // 회원가입 1000
    AUTH_SIGNUP_SUCCESS(true, 1000, "회원가입에 성공했습니다. 이메일 인증 후 로그인 해주세요"),
    AUTH_SIGNUP_FAIL_USER_ALREADY_EXITS(false, 1001, "이미 다른 계정이 존재합니다." ),
    AUTH_SIGNUP_FAIL_USER_TYPE_INVALID(false, 1002, "유효하지 않은 회원가입입니다. "),
    AUTH_SIGNUP_FAIL_NOT_COMPANY_AUTH(false, 1003, "기업 인증을 하지 않은 사용자는 가입할 수 없습니다."),
    AUTH_SIGNUP_FAIL_REGISTER_AS_SEEKER(false, 1004, "이미 지원자 회원으로 가입된 계정입니다. 지원자 회원은 채용담당자 회원으로 회원가입 할 수 없습니다."),
    AUTH_SIGNUP_FAIL_REGISTER_AS_RECRUITER(false, 1005, "이미 채용담당자 회원으로 가입된 계정입니다. 채용담당자 회원은 지원자 회원으로 회원가입 할 수 없습니다."),

    // 이메일 검증 1100
    AUTH_EMAIL_VERIFY_SUCCESS(true, 1100, "이메일 검증을 완료했습니다."),
    AUTH_EMAIL_VERIFY_FAIL(false, 1101, "이메일 검증을 실패했습니다."),
    AUTH_EMAIL_VERIFY_FAIL_USER_NOT_FOUND(false, 1102, "해당 유저를 찾을 수 없습니다."),
    AUTH_EMAIL_VERIFY_FAIL_USER_TYPE_INVALID(false, 1103, "유효하지 않은 이메일 검증입니다."),
    ...
    AUTH_RESTORE_SUCCESS(true, 1510, "비활성화된 계정입니다. 복구 절차로 전송된 이메일 인증을 완료 해주세요"),

    // 사용자 조회 1600
    AUTH_READ_SUCCESS(true, 1600, "사용자 정보를 조회하는데 성공했습니다."),
    AUTH_READ_FAIL_SEEKER_INFO_NOT_FOUND(false, 1601, "지원자 정보를 조회하는데 실패했습니다."),
    AUTH_READ_FAIL_RECRUITER_INFO_NOT_FOUND(false, 1602, "채용담당자 정보를 조회하는데 실패했습니다."),
    AUTH_READ_FAIL_ESTIMATOR_INFO_NOT_FOUND(false, 1603, "면접관 정보를 조호히하는데 실패했습니다."),
    ...
    RESUME_REGISTER_FAIL_NOT_FOUND_MEMBER(false, 2004, "해당 유저를 찾을 수 없습니다."),
    ...
    RESUME_SUBMIT_SUCCESS(true, 2010, "해당 공고에 지원서를 제출했습니다." ),

    // 지원서 조회 2100
    RESUME_READ_SUCCESS(true, 2100, "지원서 조회에 성공하였습니다." ),
    RESUME_READ_FAIL(false, 2101, "지원서 조회에 실패하였습니다." ),
    RESUME_READ_FAIL_INVALID_REQUEST(false, 2102, "해당 지원서에 대한 조회 권한이 없습니다."),
    RESUME_READ_FAIL_NOT_REGISTER_INTEGRATED_RESUME(false, 2103, "통합 지원서를 등록하지 않았습니다. 먼저 등록해주세요." ),
    RESUME_READ_FAIL_NOT_REGISTER_SUBMISSION_RESUME(false, 2104, "해당 공고에 지원한 기록이 존재하지 않습니다." ),
    RESUME_READ_FAIL_COMPANY_NOT_FOUND(false, 2106, "기업 조회에 실패하였습니다." ),
    RESUME_READ_SUCCESS_RESUMED_MEMBER(true, 2110, "지원자 조회에 성공하였습니다." ),
    ...
    RESUME_UPDATE_FAIL_DOC_PASSED_ALREADY(false, 2203, "이미 처리되었습니다." ),
    ...
    ANNOUNCEMENT_REGISTER_STEP_TWO_FAIL_INVALID_REQUEST(false, 3104, "해당 공고의 채용 담당자가 아닙니다."),

    // 공고 조회 3200
    ANNOUNCEMENT_READ_ALL_SUCCESS(true, 3200, "공고 전체조회에 성공했습니다."),
    ANNOUNCEMENT_READ_SUCCESS(true, 3210, "공고 조회에 성공했습니다."),
    ANNOUNCEMENT_READ_FAIL(false, 3201, "공고 조회에 실패했습니다."),
    ANNOUNCEMENT_READ_FAIL_NOT_FOUND(false, 3202, "해당 공고를 찾을 수 없습니다."),
    ANNOUNCEMENT_READ_FAIL_INVALID_REQUEST(false, 3203, "해당 공고의 채용 담당자가 아닙니다."),
    ANNOUNCEMENT_READ_CUSTOM_FORM_FAIL(false, 3204, "조립된 지원서 폼이 없습니다." ),
    ANNOUNCEMENT_READ_CUSTOM_FORM_SUCCESS(true, 3220, "조립된 지원서 폼 조회에 성공했습니다." ),
    ...
    COMPANY_REGISTER_SUCCESS(true, 4000, "기업 정보 등록에 성공했습니다."),
    COMPANY_READ_INFO(true, 4001, "기업 필수정보 조회에 성공했습니다."),
    ...
    COMPANY_REGISTER_FAIL_NOT_FOUND_VERIFICATION(false, 4006, "기업 인증 정보를 찾을 수 없습니다."),
    ...
    INTERVIEW_SCHEDULE_CREATE_FAIL_INVALID_REQUEST(false, 1308, "해당 공고의 채용담당자가 아니므로 인터뷰 일정을 생성할 수 없습니다"),
